...
def clean_graph():
    """Reset the default graph before every test."""
...
class TestInputConnections:
    def test_const_has_no_inputs(self):
        n = vf.make_const(t(np.array([1.0])))
        assert len(n.inputs) == 0

    def test_add_has_two_inputs(self):
        a = vf.make_const(t(np.array([1.0])))
        b = vf.make_const(t(np.array([2.0])))
        c = vf.make_add(a, b)
        assert len(c.inputs) == 2

    def test_relu_has_one_input(self):
        a = vf.make_const(t(np.array([1.0])))
        r = vf.make_relu(a)
        assert len(r.inputs) == 1

    def test_input_identity_preserved(self):
...
class TestNodeNames:
    def test_auto_name_contains_type(self):
        n = vf.make_add(
...
class TestEvaluatedFlag:
    def test_new_node_not_evaluated(self):
        n = vf.make_const(t(np.array([1.0])))
        assert not n.evaluated

    def test_op_nodes_not_evaluated(self):
        a = vf.make_const(t(np.array([1.0])))
        b = vf.make_const(t(np.array([2.0])))
        c = vf.make_add(a, b)
...
class TestGraphStructure:
    def test_single_const_registered(self):
    ...
    def test_repr_contains_type_and_name(self):
...
class TestDAGTopology:
    ...
    def test_shared_input_same_object(self):
