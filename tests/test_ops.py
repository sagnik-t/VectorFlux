...
class TestAdd:
    ...
    def test_result_is_new_tensor(self, t):
    ...
    def test_shape_mismatch_raises(self, t):
        a = vf.Tensor([2, 3])
        b = vf.Tensor([2, 4])
        with pytest.raises(Exception, match="shape mismatch|Shape"):
            vf.add(a, b)

    def test_rank_mismatch_raises(self, t):
        a = vf.Tensor([6])
        b = vf.Tensor([2, 3])
...
class TestRelu:
    ...
    def test_does_not_mutate_input(self, t):
        na = np.array([-1, 2, -3], dtype=np.float32)
        a = t(na)
        _ = vf.relu(a)
...
class TestMatmul:
    ...
    def test_vector_dot_product(self, t):
